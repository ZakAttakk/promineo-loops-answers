...
var f = 3
console.log(girlsNames[f]) // This will print the fourth item in the array.  Here we're using a variable as an index number.
...
// #5: Using the For Loop below, only log girls names that are exactly 6 characters long.  If a name is less than 6 characters long, log "That name is too short".  If a name is greater than 6 characters long, log "That name is too long."  One exception: If the name is "Penelope", log "That name is long but still very nice."

for (let i = 0; i < girlsNames.length; i++) {
    var currentName = girlsNames[i];
    if (currentName.length == 6){
        console.log(currentName)
    } else if (currentName.length < 6) {
        console.log(currentName + " is too short")
    } else if (currentName.length > 6) {
        if (currentName == "Penelope"){
            console.log(currentName + " is too long but still very nice.")
        } else {
            console.log(currentName + " is too long")
        }
    }
    if (i == girlsNames.length - 1) {
        var answerToPrompt = prompt("Are you happy?")
        if (answerToPrompt == "yes") {
